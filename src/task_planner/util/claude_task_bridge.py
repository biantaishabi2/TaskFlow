...
import re
import time
# Simple mock implementation since the actual bridge is not available
class LLMBridge:
    ...
    def get_decision(self, output, max_tokens=150):
        return "continue"
...
class ClaudeBridge:
    ...
    def llm_controlled_session(self, initial_prompt, max_turns=10, timeout=300):
        return [{"role": "claude", "content": "Mock Claude response", "timestamp": time.strftime("%Y-%m-%d %H:%M:%S")}]
