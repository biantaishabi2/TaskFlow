    ...
    def register_tool(self, 
        ...
        def read_file(path: str) -> Dict[str, Any]:
            ...
            try:
                path = Path(path).resolve()
                if not path.exists():
                    return {"success": False, "error": f"文件不存在: {str(path)}"}
        ...
        # 修改文件
        def modify_file(path: str, original_snippet: str, new_snippet: str) -> Dict[str, Any]:
            """修改文件内容"""
            try:
                path = Path(path).resolve()
                if not path.exists():
                ...
                if original_snippet not in content:
                    return {"success": False, "error": "原始内容片段未找到"}
                    
                updated = content.replace(original_snippet, new_snippet, 1)
        ...
        register_function(
            ...
            caller=caller,
            executor=executor,
            description="创建文件并写入内容。参数：path (文件路径), content (文件内容)"
        )
        ...
        register_function(
            ...
            caller=caller,
            executor=executor,
            description="修改文件内容。参数：path (文件路径), original_snippet (原始内容片段), new_snippet (新的内容片段)"
        )
